...
<!-- 다른 프레임워크를 선택한 경우 이 ADR을 교체하세요.
예: "React + Vite 선택" / "Express + React 선택" -->
...
<!-- DB가 필요한 앱이라면 이 ADR을 교체하세요.
예: "ADR-003: Prisma + PostgreSQL 선택" -->
...
### ADR-00N: [결정 제목]
**결정**: [무엇을 어떻게 하기로 했는지 1줄]

**이유**: [왜 이 선택을 했는지 — 다른 선택지와 비교해 이유를 구체적으로]

**트레이드오프**: [이 선택의 단점, 그리고 어떻게 완화하는지]
-->
